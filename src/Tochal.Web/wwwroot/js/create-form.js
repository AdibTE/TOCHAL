...
function newGalleryImage() {
    let input = `<input type="file" class="hide lastInputGallery" name="galleryImage" onchange="lastInputHandle()">`;
    $('form').append(input);
    $('.lastInputGallery').click()
}
function lastInputHandle(e) {
    var url = URL.createObjectURL(document.querySelector('.lastInputGallery').files[0]);
